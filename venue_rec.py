...
class VenueRec(object):
    ...
    def crawl_songkick(self, sk_artists):
        con = sql3.connect(db_path)
        df_existing_artists = pd.read_sql(
            "select * from artists", con, index_col='id'
        )['name'].to_dict()
        for i in range(0, len(sk_artists)):
            artist_id = sk_artists[i]
            # if int(artist_id) in df_existing_artists:
            if int(artist_id) in df_existing_artists:
            ...
            try:
                ...
                continue
            while 'event' in events['resultsPage']['results']:
    ...
    def get_similar(self, sk_artist_id):
        ...
        insert_query = """
            INSERT OR IGNORE INTO similar_artists (artist_id, similar_artists, count)
            VALUES (?, ?, ?);
        """
        con = sql3.connect(db_path)
        c = con.cursor()
        c.execute(
            insert_query,
            (sk_artist_id, ", ".join(map(str, sim_artist_ids)), len(sim_artist_ids))
        )
        con.commit()
        con.close()
    ...
    def get_venues(self, top_venues, num=20, exclude_recent=False, sk_artist_id=None):
        ...
        if exclude_recent and sk_artist_id is not None:
            ...
            top_venues_df = top_venues_df.loc[
                ~top_venues_df['id'].isin(recent_venues['id']), :
        ...
        return venues.merge(top_venues_df).sort_values(by="score", ascending=False)
