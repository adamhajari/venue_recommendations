...
class VenueRecApp(server.App):
	...
	def getData(self, params):
		...
		if self.params_cache!=params:	# caching layer
			...
			top_venues_df = top_venues_df.loc[(top_venues_df['lng']>-130) & (top_venues_df['lat']<55),:]
	...
	def html_map(self,params):
		...
		p = figure(title=title, toolbar_location="right", plot_width=600, plot_height=400, tools='hover,box_zoom,reset', title_text_font_size='12pt')
	...
	def attribution(self,params):
		html = "I can take up to a minute to load the data for an artist for the first time. Be patient!<br>"
